...
function main(canvas, context, device) {
  ...
  const c = new Square({ device, pipeline, size: 50, rgb: [1, 0, 0] });
  c.position.x = 100;
  c.position.y = 400;

  const d = new Square({ device, pipeline, size: 50, rgb: [1, 0, 0] });
  d.position.x = 100;
  d.position.y = 100;

  const e = new Square({ device, pipeline, size: 50, rgb: [1, 0, 0] });
  e.position.x = 400;
  e.position.y = 100;
  ...
  function render() {
    requestAnimationFrame((current_times_ms) => {
      ...
      c.update_world_matrix();
      c.render({ device, pass });
      d.update_world_matrix();
      d.render({ device, pass });
      e.update_world_matrix();
      e.render({ device, pass });
